...
- Treat `.antigravity/tasks/items/*.md` as canonical orchestration state for task tracking.
- Treat `.antigravity/TASKS.md`, `.antigravity/TASK_STATUS.md`, and `.antigravity/tasks/index.json` as generated views of those task files.
- Use write-through memory: promote durable facts, preferences, and lessons into the correct memory/context files during the same session.
- Treat repeated corrections, regressions, and manual workarounds as self-improvement inputs; persist them as context updates or tagged improvement tasks instead of leaving them in chat.
- Be proactively useful inside safe boundaries: continue low-risk local work, surface blockers early, and ask only for destructive, external, or materially surprising actions.
...
<memory_and_autonomy>
Automatic memory, self-improvement, and proactive-action rules are defined in `.antigravity/AGENT_OPERATING_MODEL.md`.

Minimum contract:
- Daily/episodic memory belongs in `/Users/cderamos/.openclaw/workspace/memory/YYYY-MM-DD.md`.
- Stable long-term facts belong in `/Users/cderamos/.openclaw/workspace/MEMORY.md`.
- Relationship/workflow rules belong in `docs/context/RELATIONSHIP.md`, `docs/context/USE_CASES.md`, and `docs/context/LEARNING_SYSTEM.md`.
- Real implementation, improvement, or follow-up work belongs in `.antigravity/tasks/items/*.md`.
- If the user corrects CD twice on the same pattern, create or claim an improvement task instead of relying on intention alone.
- Safe proactive work may continue without asking when it is local, reversible, and aligned with the active task; report it in concise progress updates.
</memory_and_autonomy>
...
<task_management>
Canonical task state for this repo lives in `.antigravity/tasks/items/*.md`.

Required task workflow:
- Before substantive work, create or claim a task file.
- Update `owner_agent`, `agent_type`, `status`, and `updated_at` in the task file when state changes.
- Append progress and decisions in the task file `## Activity Log`.
- After editing task files, regenerate the board/index views with:
...
Ownership model:
- `CD` is the default orchestrator and router.
- Specialist agents should claim tasks by setting `owner_agent` and `agent_type`.
- Do not use chat history as task state when a task file should be updated instead.
</task_management>
