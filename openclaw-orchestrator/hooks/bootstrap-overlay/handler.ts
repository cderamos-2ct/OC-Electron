...
export default async function handler(event) {
  ...
  const overlayAgents = await loadBootstrapFile(
    path.join(workspaceDir, ".antigravity", "AGENTS.md"),
    "AGENTS.md",
  );
  const overlayTools = await loadBootstrapFile(
    path.join(workspaceDir, ".antigravity", "TOOLS.md"),
    "TOOLS.md",
  );
  const operatingModel = await loadBootstrapFile(
    path.join(workspaceDir, ".antigravity", "AGENT_OPERATING_MODEL.md"),
    "AGENT_OPERATING_MODEL.md",
  );
  const profileId = resolveAgentProfileId(event);
  const profileSoul = profileId
    ? await loadBootstrapFile(
        path.join(workspaceDir, ".antigravity", "agents", "profiles", profileId, "SOUL.md"),
        `SOUL.md (${profileId})`,
      )
    : null;
  const profileMemory = profileId
    ? await loadBootstrapFile(
        path.join(workspaceDir, ".antigravity", "agents", "profiles", profileId, "MEMORY.md"),
        `MEMORY.md (${profileId})`,
      )
    : null;
  const profileHeartbeat = profileId
    ? await loadBootstrapFile(
        path.join(workspaceDir, ".antigravity", "agents", "profiles", profileId, "HEARTBEAT.md"),
        `HEARTBEAT.md (${profileId})`,
      )
    : null;
  const profileDirectives = profileId
    ? await loadBootstrapFile(
        path.join(workspaceDir, ".antigravity", "agents", "profiles", profileId, "DIRECTIVES.md"),
