...
AVAILABLE TOOLS (you have Bash access):
  ...
  🌐 Google Workspace CLI (gws — covers Gmail, Calendar, Chat, Drive, Tasks, People):
    ...
    gws calendar events list --params '{{"calendarId":"primary","timeMin":"...","timeMax":"..."}}'
  ...
  📧 Email (legacy — use gws gmail when available):
    python3 -W ignore {google_helper} gmail_unread [limit]
    {scripts_dir}/get_recent_mail.sh [limit]
  📅 Calendar (legacy — use gws calendar when available):
  ...
  🔥 Fireflies (meeting transcripts):
    curl -s -X POST https://api.fireflies.ai/graphql -H "Authorization: Bearer $(cat /Volumes/Storage/OpenClaw/.secrets/fireflies_api_key.txt)" -H "Content-Type: application/json" -d '{{"query":"{{ transcripts {{ id title date duration }} }}"}}'

  🤖 Agent Dispatch (inter-agent messaging):
    /Volumes/Storage/OpenClaw/.antigravity/runtime/dispatch.sh <from> <to> <action> <context>
    Agent inboxes: /Volumes/Storage/OpenClaw/.antigravity/runtime/dispatches/<agent-id>/
    Agent status: /Volumes/Storage/OpenClaw/.antigravity/runtime/agent-status.json

AGENT TEAM (13 agents):
  🧠 CD (orchestrator) | 🛡️ Karoline (comms commander) | 🌈 Iris (channel aggregator)
  🪶 Hermes (people intelligence) | 🏠 Vesta (family/personal) | 🏛️ Marcus (finance)
  🔮 Ada (notes/knowledge) | ⏳ Kronos (calendar) | 👁️ Argus (ops)
  🔥 Vulcan (build) | 📡 Hypatia (research) | 🏛️ Socrates (learning) | ⚖️ Themis (verification)

DISPATCH ROUTING (Karoline decides, then dispatches):
  Financial emails → Marcus | Meeting recaps → Ada | Schedule requests → Kronos
  System alerts → Argus | Tech specs → Vulcan | Vendor research → Hypatia
  Learning moments → Socrates | Verification → Themis
...
Use [DELEGATE] and [MESSAGE] directives to route work to the right agent.
...
def run_heartbeat():
    ...
    # Post-cycle: auto-unblock check
    try:
        scripts_dir = BASE_DIR / "scripts"
        unblock_script = scripts_dir / "auto-unblock.py"
        if unblock_script.exists():
            subprocess.run(
                ["python3", str(unblock_script)],
    ...
    # Post-cycle: SLA check for overdue dispatches
    try:
        sla_script = scripts_dir / "check-sla.py"
        if sla_script.exists():
            subprocess.run(
                ["python3", str(sla_script), "--quiet"],
                capture_output=True, text=True, timeout=30,
                env={**os.environ, "CLAUDECODE": "", "CLAUDE_CODE_ENTRYPOINT": ""},
            )
    except Exception:
    ...
    # Post-cycle: memory extraction on heartbeat response
    if response:
        try:
            processor = Path("/Volumes/Storage/OpenClaw/.antigravity/memory/auto-remember-processor.py")
            if processor.exists():
                subprocess.run(
                    ["python3", str(processor), "--line", response[:500], "--session", "heartbeat"],
                    capture_output=True, text=True, timeout=30,
                    env={**os.environ, "CLAUDECODE": "", "CLAUDE_CODE_ENTRYPOINT": ""},
                )
        except Exception:
