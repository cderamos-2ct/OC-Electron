        ...
        try:
            ...
            if name == "CREATE_CRON":
                ...
                if session_target == "isolated":
                    # Isolated autonomous jobs should not rely on channel delivery defaults.
                    # The repo-owned autonomy bridge injects visible results into webchat.
                    cron_params["delivery"] = {"mode": "none"}
            ...
            if name == "AUTO_REMEMBER":
                directive_line = args.get("line", line).strip() or line
                session_id = args.get("session", source).strip() or source
                try:
                    processor = _storage_root() / ".antigravity" / "memory" / "auto-remember-processor.py"
                    subprocess.run(
                        [
                            "python3",
                            str(processor),
                    ...
                    applied.append(f"auto-remember:{directive_line[:40]}")
                except Exception:
                    applied.append("directive_error:auto-remember")
                continue

            if name == "LEARN_RULE":
                title = args.get("title", "").strip()
                rule = args.get("rule", "").strip() or title
                if title and rule:
                    file_path = _storage_root() / ".antigravity" / "memory" / "LEARNED_RULES.md"
                    file_path.parent.mkdir(parents=True, exist_ok=True)
                    current = file_path.read_text(encoding="utf8") if file_path.exists() else "# Learned Rules\n\n"
                    entry = f"- {now.isoformat()} [{source}] **{title}**: {rule}"
                    if entry not in current:
                        file_path.write_text(current.rstrip() + f"\n{entry}\n", encoding="utf8")
                    applied.append(f"learn-rule:{title[:40]}")
                continue

            if name == "GRADUATE":
                item = args.get("item", "").strip()
                target = args.get("to", "longterm").strip().lower() or "longterm"
                summary = args.get("summary", "").strip() or item
                file_path = _promotion_target_path(target, now)
                if item and file_path:
                    _append_runtime_entry(file_path, f"[GRADUATED] {summary}", now)
                    applied.append(f"graduate:{item[:40]}:{target}")
                continue
