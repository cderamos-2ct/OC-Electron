...
# Configure gateway URL (optional)
...
```
NEXT_PUBLIC_OPENCLAW_GATEWAY_URL=ws://127.0.0.1:3000/__mounted/control
NEXT_PUBLIC_OPENCLAW_GATEWAY_TOKEN=your-token-here
```

The shell server also supports mounted sibling dashboards and a same-origin control-ui mount:
...
`18792` is the recommended default for `openclaw-agent-dashboard` in this setup because OpenClaw itself already uses `18791` for the browser control server.
